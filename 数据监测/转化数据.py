...
class Conversion_Data:
    ...
    def select_data(self, date, hour):
        sql1 = f''' -- 订单&风控信息  近10日数据   
                ...
                and  date_format(om.create_time, '%Y-%m-%d')>='{date}'# TODO
                -- and date_format(om.create_time, '%Y-%m-%d')<='2025-11-15'
    ...
    def my_job(self, hour, minute, path, hours):
        ...
        print(df[df.order_number == 'A20251122145553722'][['status2', '机型内存', '机型', 'merchant_name']])
        # 总体
        ...
        df_model_group_now_ss, df_model_group_7_ss, df_model_group_15_ss = self.conversion_model(df_ss, df2_ss, df_risk_examine, '机型内存')
        model_list_ss = df_model_group_15_ss.sort_values(by='去重订单数', ascending=False).head(10).index.to_list()
        self.data_tips(df_ss, df2_ss, df_risk_examine, '机型内存', model_list_ss, '搜索渠道')
        ...
        id_list_zm = [8592, 8593, 8594, 8595, 8600, 8601, 8602, 8603, 8558, 8559, 8684, 8696, 8620, 8679,8688, 8803,
                    7946, 7960, 8771, 8772, 8234, 8349, 8801, 9046, 9049, 7931, 7933, 7934, 7935, 7936, 7937, 7938, 7939,
                    7940, 8724, 8725,8726, 6712, 6713, 7681, 7682, 7684, 6687, 6686]
        id_list_ss = [8608,8609,8563,8564,8565,8566,8654]
        # 芝麻
        df_zm_uv = df[df.商品_ID.isin(id_list_zm[:])]
        df2_zm_uv = df2[df2.商品_ID.isin(id_list_zm[:])]
        # print('芝麻商品ID:', df_zm_uv.商品_ID)
        df_zm_uv, df_zm_uv_7, df_zm_uv_15 = self.conversion_model(df_zm_uv, df2_zm_uv, df_risk_examine, '商品ID')
        # 搜索
        df_ss_uv = df[df.商品_ID.isin(id_list_ss[:])]
        df2_ss_uv = df2[df2.商品_ID.isin(id_list_ss[:])]
        df_ss_uv, df_ss_uv_7, df_ss_uv_15 = self.conversion_model(df_ss_uv, df2_ss_uv, df_risk_examine, '商品ID')
        self.data_tips(df, df2, df_risk_examine, '商品_ID', id_list_zm[:], '商品ID_芝麻租物')
        self.data_tips(df, df2, df_risk_examine, '商品_ID', id_list_ss[:], '商品ID_搜索渠道')
        print('商品ID转化数据计算完毕！')
        # 芝麻严选仅商品ID不分必发非必发
        zmyx_list = [8946, 8947, 8948, 8949, 8950, 8951, 8952, 8953, 8954, 8955, 8956, 8957, 8958, 8959,8734,8733,8759,8758,7682,7681, 7934, 7933, 7931, 7937, 7936, 7935, 7938, 7939, 7940]
        df_zmyx = df[df.商品_ID.isin(zmyx_list[:])]
        df2_zmyx = df2[df2.商品_ID.isin(zmyx_list[:])]
        df_zmyx, df_zmyx_7, df_zmyx_15 = self.conversion_model(df_zmyx, df2_zmyx, df_risk_examine, '商品ID')
        self.data_tips(df, df2, df_risk_examine, '商品_ID', zmyx_list[:], '商品ID_严选')
        ...
        with pd.ExcelWriter(path + f'机型转化/近15日转化数据_搜索渠道_{today}.xlsx', engine='xlsxwriter') as writer:
            df_model_group_now_ss.to_excel(writer, sheet_name='当日机型转化')
            df_model_group_7_ss.to_excel(writer, sheet_name='近7日机型转化')
            df_model_group_15_ss.to_excel(writer, sheet_name='近15日机型转化')
        ...
        with pd.ExcelWriter(path + f'长租严选商品/近15日芝麻转化数据_商品ID_严选_{today}.xlsx', engine='xlsxwriter') as writer:
            df_zmyx.to_excel(writer, sheet_name='当日商品ID转化')
            df_zmyx_7.to_excel(writer, sheet_name='近7日商品ID转化')
            df_zmyx_15.to_excel(writer, sheet_name='近15日商品ID转化')
        print('数据写入完毕！')
    ...
    def zfb_order(self, hour, minute, path, hours):
        ...
        print(f'执行定时任务：现在是{today}的{hour}点{minute}分...')
        print('正在查询数据...')
        df_all, df_risk, df_risk_examine, df_re, df_ra = self.run(date, hours)
        print('数据查询完毕！\n正在清理数据...')
        # 不去重支付宝数据 2025-12-22
        df, df2 = self.clean_data(df_all, df_risk, df_re, df_ra)
        df_bqc, df2_bqc = self.clean_data_bqc(df_all, df_risk, df_re, df_ra)
        print('数据清理完毕！\n获取支付宝订单数据...')

        df_zfb = df[df.order_type=='ZFB_ORDER']
        df2_zfb = df2[df2.order_type=='ZFB_ORDER']

        df_bqc_zfb = df_bqc[df_bqc.order_type=='ZFB_ORDER']
        df2_bqc_zfb = df2_bqc[df2_bqc.order_type=='ZFB_ORDER']
        df_bqc_zw = df_bqc[df_bqc.归属渠道 == '芝麻租物']
        df2_bqc_zw = df2_bqc[df2_bqc.归属渠道 == '芝麻租物']
        df_bqc_group = self.all_model.data_group(df_bqc_zfb, df2_bqc_zfb, df_risk_examine, '下单日期')
        df_bqc_group = df_bqc_group[["去重订单数", "前置拦截", "拦截率", "进件数", "预授权通过率", "机审强拒", "强拒比例", "机审通过件",
                                "人审拒绝", "风控通过件", "风控通过率", "客户取消", "无法联系",
                                "出库前风控强拒", "待审核", '出库', '拒量出库', '进件出库率',
                                '总体进件出库率（含拒量）', '拒量进件出库率增加', '取消率', '人审拒绝率', '出库前强拒比例', '无法联系占比', '订单出库率']].fillna(0)
        df_bqc_zw_group = self.all_model.data_group(df_bqc_zw, df2_bqc_zw, df_risk_examine, '下单日期')
        df_bqc_zw_group = df_bqc_zw_group[["去重订单数", "前置拦截", "拦截率", "进件数", "预授权通过率", "机审强拒", "强拒比例", "机审通过件",
                                "人审拒绝", "风控通过件", "风控通过率", "客户取消", "无法联系",
                                "出库前风控强拒", "待审核", '出库', '拒量出库', '进件出库率',
                                '总体进件出库率（含拒量）', '拒量进件出库率增加', '取消率', '人审拒绝率', '出库前强拒比例', '无法联系占比', '订单出库率']].fillna(0)
        # df_zfb = df[df.归属渠道=='芝麻租物']
        # df2_zfb = df2[df2.归属渠道 == '芝麻租物']
        ...
        with pd.ExcelWriter(path + f'支付宝订单/近1个月支付宝订单数据_{today}.xlsx', engine='xlsxwriter') as writer:
            ...
            df_bqc_group.to_excel(writer, sheet_name='支付宝订单不去重')
            df_bqc_zw_group.to_excel(writer, sheet_name='芝麻租物不去重')
        print('数据写入完毕！')
    ...
    # 筛选非二手数据
        ...
        # 对数据进行分组统计
            df_model_group = self.all_model.data_group(df[:-1], df2[:-1], df_risk_examine, '机型')# 机型内存
        # 计算出库量（包含拒量出库）
            df_model_group.loc[:, '出库'] = df_model_group.出库 + df_model_group.拒量出库
        ...
        df_ss = df[df.归属渠道 == '搜索渠道']
        df2_ss = df2[df2.归属渠道 == '搜索渠道']
        df_model_group_ss = channel(df_ss, df2_ss)
        print('机型转化数据计算完毕！\n正在写入数据...')
        with pd.ExcelWriter(path + f'机型转化/月度机型转化_{today}.xlsx', engine='xlsxwriter') as writer:
            ...
            df_model_group_ss.to_excel(writer, sheet_name='搜索渠道月度机型转化')
        print('数据写入完毕！')
