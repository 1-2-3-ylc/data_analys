...
class Conversion_Data:
    ...
    def jd_job(self, hour, minute, path, hours):
        ...
        sql_jd = ''' -- 京东外部订单关联表
                    ...
                    where tojo.create_time>=date_add(current_date, interval - 15 day) and tojo.create_time<current_date
                '''
        ...
        print('数据清理完毕！\n正在计算数据...')
        # 关联数据
        df_jd_new = df_jd.merge(df[['order_id', 'new_actual_money']], on='order_id', how='left')
        # 区分京东渠道并计算进件和风控通过的件数
        df = df[df.归属渠道=='京东渠道']
        ...
        # 获取京东的不去重买断价
        sql_jd_md = '''
        select distinct o.order_number, od.actual_money 买断价, tojr.create_time from db_digua_business.t_order as o 
        ...
        where tojr.create_time>=date_add(current_date, interval -15 day)
    ...
    def zfb_order(self, hour, minute, path, hours):
        ...
        print('提醒信息获取完毕！\n正在写入数据...')
        today = datetime.now().strftime('%Y%m%d')
        with pd.ExcelWriter(path + f'支付宝订单/近15日支付宝订单数据_{today}.xlsx', engine='xlsxwriter') as writer:
            df_zfb_group[:-1].to_excel(writer, sheet_name='支付宝订单')
        print('数据写入完毕！')
        self.clean.send_dingtalk_message(self.webhook, self.secret, messages)

    # 机型转化——月度
    def month_model(self, hour, minute, path, hours):
        date = ((datetime.today() - pd.DateOffset(months=1)).replace(day=1)).strftime('%Y-%m-%d')
        today = datetime.now().strftime('%Y%m%d')
        print(f'执行定时任务：现在是每日的{hour}点{minute}分...')
        print('正在查询数据...')
...
if __name__ == '__main__':
    ...
    # cd.zfb_order(hour, minute, path, 15, 24)
    ...
    job_zfb = scheduler.add_job(cd.zfb_order, 'cron', hour=9, minute=1, args=[9, 1, path, 24])
    job_month = scheduler.add_job(cd.month_model, CronTrigger(day=1, hour=11, minute=1), args=[11, 1, path, 24])
    # cd.month_model(hour, minute, path, 24)
    print('定时任务创建完毕...\n正在执行定时任务...')
    # 查看是否添加了任务
