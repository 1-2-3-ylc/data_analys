...
class All_Model:
    ...
    def xmy(self, df, df_risk_examine, model):
        ...
        '''
        # 匹配小蚂蚁（拒绝）数据
        df_xmy = df[df['merchant_name'].isin(['小蚂蚁租机', '兴鑫兴通讯', '人人享租', '崇胜数码', '喜卓灵租机', '喜卓灵新租机', '云启德曜', '智云享租'])]
        # 出库前强拒数据重命名
    ...
    def data_group(self, df, df2, df_risk_examine, model):
        ...
        # 拒绝
        df_241205 = df_241205[~df_241205.merchant_name.isin(['小蚂蚁租机', '兴鑫兴通讯', '人人享租', '崇胜数码', '喜卓灵租机', '喜卓灵新租机', '云启德曜', '智云享租'])]
        # 过往机审拒量逻辑
        ...
        # 2025-12-03 出库前强拒逻辑修改，原有的出库前强拒量累积至机审强拒，后续环节不再出现原出库前强拒量
        df_group['机审强拒'] = df_group['机审强拒'] + df_group['小蚂蚁机审强拒'] + df_group['机审强拒_拒量'] + df_group['order_risk_ex'] + df_group['出库前强拒_拒量']
        # 总体机审通过件，包含拒量
        df_group['机审通过件'] = df_group['机审通过件'] + df_group['小蚂蚁人审拒绝']   # +除机审强拒的单
        # df_group['风控通过件'] = df_group['风控通过件']   # +出库前强拒的单
        df_group['人审拒绝'] = df_group['人审拒绝'] + df_group['小蚂蚁人审拒绝']
        # df_group['出库前风控强拒'] = df_group['出库前风控强拒'] 
...
class Data_Clean:
    ...
    def reject_type(self, a, b, c, d, e, f, g, h, i, j):
        ...
        if h == 1 and b == "未进件":
            ...
            return "出库前风控强拒"
        elif b == "进件" and pd.notna(c) and '已退款' in e and j != 1:# and j != 1 2025-12-03人审拒绝不含发货前强拒，冗余逻辑
            return "人审拒绝"
        elif b == "进件" and pd.notna(d) and str(d).strip():
            return "客户取消"
        elif b == "进件" and pd.notna(f) and str(f).strip():
            return "无法联系"
        # elif b == "进件" and j == 1:# 出库前强拒逻辑更改，发货前强拒隐含在人审拒绝中，故调整顺序至人审拒绝前2025-12-03
        #     return "出库前风控强拒"
    ...
    def drop_merchant(self, df):
        ...
        # df.drop(df[df['merchant_name'] == "云启德曜"].index, inplace=True) # 拒量谦莲科技
        df.drop(df[df['merchant_name'] == "艾欧尼亚数码"].index, inplace=True)
        df.drop(df[df['merchant_name'] == "谦莲科技"].index, inplace=True)
    ...
    def drop_rejected_merchant(self, df):
        ...
        # 将多行drop操作合并为一行
        reject_merchants = ["小蚂蚁租机", "兴鑫兴通讯", "人人享租", "崇胜数码", "喜卓灵租机", "喜卓灵新租机", "云启德曜", "智云享租"]
    ...
    def qudao_type(self, ly_channel, activity_name, order_method, channel_type_id, order_type):
        ...
        if (channel_type_id in [2, 3, 80] and order_type=='ZFB_ORDER')  or order_method == 1:
            ...
            return "搜索渠道"
        # channel_type_id在（48，81）且order_type不为ZFB_ORDER且ly_channel改为ly_channel+order_type 2025-11-20
        elif channel_type_id in [48, 81] and order_type not in ['ZFB_ORDER', 'WEIBO_ORDER']:
            return ly_channel+order_type
        elif order_type=='JD_ORDER':
            return '京东渠道'
        elif order_type == 'WEIBO_ORDER':
            return '新浪微博'
    ...
    def status_node(self, df):
        ...
        df["出库前风控强拒"] = np.where((df["审核状态"] == '出库前风控强拒'), 1, 0)# 只要命中就将其归入出库前风控强拒，其订单状态会动态变化
...
class Week_Model:
    ...
    def week_data_m_group(self, df, key, keys):
        ...
        if keys == '免审':
            ...
            # 2025.11.14
            week_sum_m['拦截率'] = week_sum_m['前置拦截'] / week_sum_m['去重订单数']
            week_sum_m['拦截率'] = week_sum_m['拦截率'].apply(lambda x: format(x, ".2%"))
            
            week_sum_m['预授权通过率'] = week_sum_m['进件数'] / week_sum_m['去重订单数']
            week_sum_m['预授权通过率'] = week_sum_m['预授权通过率'].apply(lambda x: format(x, ".2%"))
            # 至
            week_sum_m['强拒比例'] = week_sum_m['机审强拒'] / week_sum_m['进件数']
            week_sum_m['强拒比例'] = week_sum_m['强拒比例'].apply(lambda x: format(x, ".2%"))

            week_sum_m['风控通过率'] = week_sum_m['风控通过件'] / week_sum_m['进件数']
            week_sum_m['风控通过率'] = week_sum_m['风控通过率'].apply(lambda x: format(x, ".2%"))
            ...
            # 2025.11.14
            week_sum_m['总体进件出库率（含拒量）'] = (week_sum_m['出库'] + week_sum_m['拒量出库'])/week_sum_m["进件数"]
            # week_sum_m['拒量进件出库率增加'] = week_sum_m['总体进件出库率（含拒量）'] - week_sum_m['进件出库率']
            week_sum_m["总体进件出库率（含拒量）"] = week_sum_m["总体进件出库率（含拒量）"].apply(lambda x: format(x, ".2%"))
            # week_sum_m["拒量进件出库率增加"] = week_sum_m["拒量进件出库率增加"].apply(lambda x: format(x, ".2%"))

            week_sum_m["订单出库率"] = week_sum_m["出库"] / week_sum_m["去重订单数"]
            week_sum_m["订单出库率"] = week_sum_m["订单出库率"].apply(lambda x: format(x, ".2%"))
            # 如果存在实际购买碎屏险出库的数据，则计算实际购买碎屏险出库比例
            if '实际购买碎屏险出库' in week_sum_m.columns:
                week_sum_m['实际购买碎屏险出库比例'] = week_sum_m['实际购买碎屏险出库'] / week_sum_m['出库']
                week_sum_m['实际购买碎屏险出库比例'] = week_sum_m['实际购买碎屏险出库比例'].apply(lambda x: format(x, ".2%"))
