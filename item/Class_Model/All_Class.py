...
class All_Model:
    ...
    def product_model(self, df, df2, keys):
        ...
        df_group1 = df.groupby(['型号内存']).agg({'order_id': 'size', '是否进件': 'sum'})
        df_group2 = df2.groupby(['型号内存']).agg(
        ...
        df_group = pd.merge(df_group1, df_group2, on='型号内存', how='inner')
