...
class Cancel_Order:
    ...
    def get_data(self):
        ...
        return df_order, df_risk, df_re, df_ra
    ...
    def clean_data(self, df, df_risk, df_re, df_ra):
        ...
        df.loc[:, "状态编码"] = df["status2"].map(dict_status_code)
    ...
    def cancel(self, df, days1, days2):
        ...
        order_qx_all = jsqj_orders.query('status2=="已退款"')[['create_time', 'order_number', 'user_mobile', 'cancel_reason', 'rejected', 'total_describes']]
    ...
    def run(self):
        print('正在查询数据...')
        df, df_risk, df_re, df_ra = self.get_data()
        print('数据查询完毕...\n正在进行数据清理...')
        df_new = self.clean_data(df, df_risk, df_re, df_ra)
        print('数据清理完毕...\n正在获取数据分组...')
