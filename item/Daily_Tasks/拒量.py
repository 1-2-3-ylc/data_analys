...
class Rejected_Number:
    ...
    def clean_data(self, df, df_ck, df_risk, df_re, df_ra):
        ...
        # 删除重复订单
        df.drop_duplicates(subset=["order_id"], inplace=True)
        df.drop_duplicates(subset=["true_name", "user_mobile", "id_card_num", "下单日期"], keep="last",
                           inplace=True)
    ...
    def sort_2(self, df, df_stages, df_name_new):
        ...
        df_jl_stages_new.to_excel('F:/df_jl_stages_new.xlsx')
...
if __name__ == '__main__':
    ...
    rn = Rejected_Number()
    # rn.my_job_sort(hour, minute, path)
    print('正在创建定时任务...')
